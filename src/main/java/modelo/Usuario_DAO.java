...
public class Usuario_DAO {
	...
	public boolean insertarusuario(Usuario_DTO us) {
		...
		public Usuario_DTO consultarusuario(Usuario_DTO us) {
			...
			try {
			    ...
			    if(rs.next()) {
			     usu= new Usuario_DTO(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5), rs.getString(6));
			    }
	...
	public boolean actualizar(Usuario_DTO usu) {
		...
		int x;
		try {
			ps=cnn.prepareStatement("UPDATE usuarios SET Nombre=?, Apellido=?, Correo=?, Usuario=?, Contraseña=? WHERE documento=?");
