...
public class Productos_DAO {
	...
	public Productos_DTO consultarproducto(Productos_DTO produc) {
		...
		try {
			...
			if (rs.next()) {
				producto = new Productos_DTO(rs.getInt(1), rs.getString(2), rs.getInt(3), rs.getDouble(4),
	...
	public boolean actualizar(Productos_DTO produc) {
		...
		try {
			ps = cnn.prepareStatement(
					"UPDATE productos nombre_producto=?,  SET NIT_proveedor=?, precio_compra=?, iva_compra=?, precio_venta=? WHERE Codigo=?");
