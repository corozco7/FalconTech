...
public class Reporte_DAO {
	...
	public ArrayList<Usuario_DTO> listadousuarios(){
		...
		try {
			...
			rs=ps.executeQuery();
			while (rs.next()) {
				Usuario_DTO us= new Usuario_DTO(rs.getInt(1),rs.getString(2), rs.getString(3), rs.getString(4),rs.getString(5), rs.getString(6));
				lista.add(us);
			}
	...
	public ArrayList<Cliente_DTO> listadoclientes(){
		...
		try {
			...
			rs=ps.executeQuery();
			while (rs.next()) {
				Cliente_DTO cli	= new Cliente_DTO(rs.getInt(1),rs.getString(2), rs.getString(3), rs.getString(4),rs.getString(5), rs.getString(6));
				lista.add(cli);
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
	...
	public ArrayList<Reporte_DTO> listadoventas(){
		...
		for(Cliente_DTO cli: clientes) {
			...
			try {
				...
				if(totalventas!=0) {
					Reporte_DTO rep=new Reporte_DTO(cli.getDocumento(),cli.getNombre()+' '+cli.getApellido(),totalventas );
					lista.add(rep);
